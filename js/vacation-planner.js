(function () {
    ...
    function renderBranchRules() {
        var branch = state.currentPlan.branch;
        var rules = branch === "alternate"
            ? [
                "Nonstop flights",
                "Any non-Frontier airline if lower than SkyTeam",
                "Frontier excluded"
            ]
            : [
                "Boston start",
        ...
        if (state.currentPlan.pmDepart) {
            rules.push("PM departures favored");
        }

        els.branchRules.innerHTML = rules.map(function (rule) {
            return "<div class=\"rule-pill\"><i class=\"fa fa-check\" aria-hidden=\"true\"></i>" + escapeHtml(rule) + "</div>";
        }).join("");
    }
    ...
    function renderRecommendation() {
        ...
        var notes = evaluation.reasons.length ? evaluation.reasons.join(", ") : "Matches the active preferences";
        ...
        els.recommendationBoard.innerHTML = [
            ...
            "</div>",
            "<p class=\"candidate-notes\">" + escapeHtml(notes) + "</p>",
            "</div>",
    ...
    function evaluateCandidate(candidate) {
        ...
        if (!destinationMatches) {
            reasons.push("different destination");
        }

        if (plan.excludeBasic && isBasic) {
            reasons.push("basic economy");
        }

        if (isFrontier) {
            reasons.push("Frontier excluded");
        }
        ...
        if (reasons.length) {
            return {
                category: "blocked",
                label: "Blocked",
                score: Math.max(5, 25 - reasons.length * 5),
                reasons: reasons
            };
        }

        if (isSkyTeam && stops <= maxStops) {
            category = "preferred";
            label = "Preferred";
            score = 82;
        } else if (!isSkyTeam && stops === 0 && !isFrontier) {
            if (benchmark && price > 0 && price < benchmark.price) {
                category = "alternate";
                label = "Alternate";
                score = 78 + Math.min(12, Math.round((benchmark.price - price) / 25));
                reasons.push("lower than SkyTeam");
            } else {
                category = "compare";
                label = "Compare";
                score = 58;
                reasons.push(benchmark ? "not lower than SkyTeam" : "needs SkyTeam benchmark");
            }
        } else {
            if (stops > maxStops) {
        ...
        if (plan.pmDepart && candidate.departTime) {
            if (isPm(candidate.departTime)) {
                score += 8;
                reasons.push("PM departure");
            } else {
                score -= 6;
                reasons.push("AM departure");
            }
        }
        ...
        if (Number(plan.budget || 0) > 0 && price > 0) {
            if (price <= Number(plan.budget)) {
                score += 8;
                reasons.push("under target fare");
            } else {
                score -= Math.min(12, Math.round((price - Number(plan.budget)) / 50));
                reasons.push("over target fare");
            }
        }
